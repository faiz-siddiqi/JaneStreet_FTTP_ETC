...
def vale(exchange):
    ...
    if buy_vale:
        ...
        if buy_vale[1] > fair - dif:
            exchange.send_cancel_message(order_id=buy_vale[0])
            buy_vale = None
    if sell_vale:
        # check if the sell price is higher than fair
        if sell_vale[1] < fair + dif:
            exchange.send_cancel_message(order_id=sell_vale[0])
            sell_vale = None
    # Buy
    if not buy_vale:
        if 10 - positions["VALE"] > 0:
            ...
            buy_vale = (order_num, min(fair - dif, prices["VALE"]["bid"] + 1))
            order_num += 1
    ...
    if not sell_vale:
        if 10 + positions["VALE"] > 0:
            ...
            sell_vale = (order_num, max(fair + dif, prices["VALE"]["ask"] - 1))
            order_num += 1
            ...
            if positions[sym] < 0:
                ...
                if not components[sym]["buy"]:
                    exchange.send_add_message(
                        ...
                        symbol=sym,
                        dir=Dir.BUY,
                        price=min(fair - 2, prices[sym]["bid"] + 2),
                        size=-positions[sym],
                    )
                    components[sym]["buy"] = (
                        order_num,
                        min(fair - 2, prices[sym]["bid"] + 2),
                        -positions[sym],
                    )
                    order_num += 1
...
def main():
    ...
    while True:
        ...
        etf(exchange)
        vale(exchange)
        if curBond != positions["BOND"]:
            bond(exchange)
            curBond = positions["BOND"]
